/* global jasmine, beforeEach, it, describe, expect */
describe("LayoutExport", function () {
	'use strict';
	describe("LayoutExportController", function () {
		var fileSystem, mapModel, currentLayout, underTest, requestId, resultPoller, observer;
		beforeEach(function () {
			fileSystem= {};
			mapModel = {};
			currentLayout = { 'a': 'b' };
		...
		it("fires a success event when the poller resolves", function () {
			var listener=jasmine.createSpy('layoutExported'), url="http://www.google.com";
			underTest.addEventListener('LayoutExportComplete', listener);
		...
		it("fires a failure event when the poller rejects", function () {

		});
		it("fires a progress event when the poller notifies", function () {

		});
		it("fires a failure event if the adapter fails to save", function () {
		});
		it("fires a start event when the export starts", function () {
		});
	});
